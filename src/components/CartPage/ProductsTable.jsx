...
const calculateTotalPrice = (item) => {
  ...
  const sumVariant = variantPrices
    ? variantPrices.reduce((sum, price) => sum + price, 0)
    : false;

  if (sumVariant) {
    const priceWithQty = sumVariant * Number(item.qty);
    return Number(item.totalPrice) + priceWithQty;
  } else {
